...
class MapDoorText:
    ...
    def parse_mdt(self, mdt_line):
        ...
        mdt_line = mdt_line.replace(
            "the limit of your vision is ", "the limit of your vision:"
        )
                ...
                if entry.startswith(tuple(exit_strings)):
                    ...
                    if data["last_count"] > 0:
                        this_square = [
                            ...
                            data["last_direction"],
                            data["room_id"],
                            int(math.floor(data["room_value"])),
                        ]
                    ...
                    if is_direction == 1:
                        if not data["ignoring_exits"]:
                            ...
                            data["last_direction"] = "{}{} {}".format(
                                data["last_direction"],
                                quantity,
                                self.direction_map[this_direction],
                            )
                        else:
                        ...
                        for custom_match in self.custom_matches:
                            if custom_match[3]:
                                ...
                                if rexp.match(entry):
                                    if (
                                        custom_match[1]
                                        and custom_match[1] in self.colour_map
                                    ):
                                        entry = "{}{}{}".format(
                        ...
                        data["room_value"] = data["room_value"] + (
                            this_value * quantity
                        )

                        if quantity > 1:
                            entry = "{} {}".format(quantity, entry)
                        data["entity_table"].append(
                            [data["room_id"], entry, data["next_color"]]
                        )

                        data["last_count"] = data["last_count"] + quantity
                        if data["last_enemy_line"] != "":
                            data["last_enemy_line"] = "{}, ".format(
                                data["last_enemy_line"]
                            )
                        data["last_enemy_line"] = "{}{}".format(
                            data["last_enemy_line"], entry
                        )
        ...
        if data["nothing"]:
            ...
            for square in data["enemies_by_square"]:
                ...
                if (
                    square[3] >= self.minimum_room_value
                    and len(square[1]) > data["longest_direction"]
                ):
            ...
            for square in data["enemies_by_square"]:
                ...
                if square[3] >= self.minimum_room_value:
                    ...
                    square[3] = "{}{}{}".format(
                        self.colour_map["cyan"], square[3], self.colour_map["reset"]
                    )
            ...
            if rooms_ignored > 0 and self.show_hidden_room_count:
                output = "({} rooms below your value limit of {})".format(
                    rooms_ignored, self.minimum_room_value
                )
                self.return_value.append(output)
